...
function extractTwitchWithCurrentTime(retried = false) {
	const shareButton = document.querySelector(
		'.channel-info-content div[data-test-selector="toggle-balloon-wrapper__mouse-enter-detector"] button:not([data-a-target="follow-button"])',
	);
	if (shareButton) {
		if (verbose === true) debugIsFun("Probably on video page");
		var tw = document.querySelector(
			'input[data-a-target="tw-input"][readonly]',
		);
		if (!tw) shareButton.click();
		document
			.querySelector('button[data-a-target="copy-link-share-button"]')
			?.click();
		const timestampToggler = document.querySelector(
			"#video-share-timestamp-toggle",
		);
		if (!timestampToggler) {
			if (!retried) setTimeout(extractURL, 100);
		...
		else console.warn("No tw input");
	} else console.warn("Share button not found");
	if (verbose === true) debugIsFun("Probably on streamer homepage");
	document.querySelector(".home a.tw-link");
	var ct = document.querySelector(
			'p[data-a-target="player-seekbar-current-time"]',
		),
		urlGenerated =
			document.querySelector(".home a.tw-link") &&
			document.querySelector(".home a.tw-link").href,
		tc = "";

	if (!ct || !urlGenerated) {
		if (verbose === true) debugIsFun("!ct || !urlGenerated");
		return;
	}

	ct = ct.innerText.split(":");

	if (ct.length >= 3) tc += ct[0] + "h";
	if (ct.length >= 2) tc += ct[1] + "m";
	if (ct.length >= 1) tc += ct[2] + "s";

	if (tc.length > 0) urlGenerated += "?t=" + tc;
...
function extractSoundcloudWithCurrentTime() {
	const lk = document.querySelector(
			".playControls .playbackSoundBadge__title a",
		),
		timev = document.querySelector(
			".playbackTimeline__timePassed span[aria-hidden]",
		);
	...
	if (!purposeUrl(urlGenerated)) return;
	return urlGenerated;
}
...
function extractGenericAudioWithCurrentTime(
	timelineSelector,
	timelineElementToSeconds = (element) => parseFloat(element.value) * 60,
) {
	const audioElement = document.querySelector("audio"),
		timelineElement = document.querySelector(timelineSelector);
...
function extractURL() {
	...
	if (window.location.href.startsWith("https://soundcloud.com/"))
		...
		window.location.href.startsWith("https://youtube.com/") ||
		window.location.href.startsWith("https://www.youtube.com/") ||
		window.location.href.startsWith("https://m.youtube.com/")
	)
		return extractYoutubeWithCurrentTime();
	else if (window.location.href.startsWith("https://www.radiofrance.fr/"))
		url = extractGenericAudioWithCurrentTime(
			'div[data-zone="COMPONENT_TIME_START"]',
			(timelineElement) => {
				const times = timelineElement.innerText
					.split(" ")[0]
					.split(":");
				if (times.length > 2)
					return (
...
function saveUrl(url) {
	if (!url) {
		if (purposeUrl(window.location.href)) url = window.location.href;
		else return;
	}
...
if (window.urlSwapperInjected !== true) {
	...
	chrome.runtime.onMessage.addListener(function (msg) {
		if (msg.action === "extractURL") extractURL();
	});
}
